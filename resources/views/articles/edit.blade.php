...
@section('title', 'Edit Artikel - PT. Maju Jaya Konstruksi')
...
<section class="py-16">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <a href="{{ route('articles.show', $article->slug) }}" class="text-blue-600 hover:text-blue-800">← Kembali ke Detail Artikel</a>
        </div>

        <h1 class="text-3xl font-bold text-gray-800 mb-8">Edit Artikel</h1>

        <form method="POST" action="{{ route('articles.update', $article->slug) }}" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                <label class="block text-gray-700 font-semibold mb-2">Gambar Utama Saat Ini</label>
                <img src="{{ Storage::url($article->featured_image) }}" alt="Featured Image" class="w-full h-64 object-cover rounded-lg mb-2">
            </div>
            @endif

            <div class="mb-6">
                <label for="featured_image" class="block text-gray-700 font-semibold mb-2">{{ $article->featured_image ? 'Ganti Gambar Utama' : 'Gambar Utama' }}</label>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('articles.show', $article->slug) }}" class="bg-gray-300 text-gray-700 px-8 py-3 rounded-lg hover:bg-gray-400 transition font-semibold">
                    Batal
                </a>
            </div>
        </form>
